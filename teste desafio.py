...
def obter_cliente_por_cpf(cpf, clientes):
    for cliente in clientes:
        if cliente.cpf == cpf:
            return cliente
    return None  # Remova a mensagem daqui
...
def criar_conta(clientes, contas):
    ...
    if not cliente:
        print("\n@@@ Cliente não encontrado! @@@")
        return  # Interrompe o fluxo caso não encontre o cliente

    numero_conta = len(contas) + 1
    limite = float(input("Informe o limite da conta (padrão é 500): ") or 500)
    limite_saques = int(input("Informe o número máximo de saques (padrão é 3): ") or 3)
